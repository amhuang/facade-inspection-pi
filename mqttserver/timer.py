...
'''
Timer that counts up, down, and in half-time.
Keeps track of time internally and returns time btwn when count_up/down
are called OR updates self.curr internally and regularly via thread that
is stopped and restarted by pause()
'''
class Timer:
    def __init__(self):
    ...
    # Sets start time. Called in countup and countdown
    def start(self):
    ...
    # Countup except counts half as fast
...
'''
Essentially same thing as setInterval in Javascript
Repeats action() every interval
'''
class setInterval:
